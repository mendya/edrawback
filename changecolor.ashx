...
Public Class delimport : Implements IHttpHandler
    ...
    Public Sub ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
		...
		Dim newsizecolor As String = json.Split("|")(4)
       
        Using connection As New SqlConnection(connectionString)
            Try
                connection.Open()
                Using Command As New SqlCommand("update import set sizecolor=@newsizecolor where company=@company and style=@style and sizecolor=@sizecolor and import_date=@import_date;", connection)
                    Command.Parameters.AddWithValue("@company", company)
                    Command.Parameters.AddWithValue("@style", style)
                    Command.Parameters.AddWithValue("@sizecolor", sizecolor)
					Command.Parameters.AddWithValue("@newsizecolor", newsizecolor)
                    Command.Parameters.AddWithValue("@import_date", importdate)
                    rows = Command.ExecuteNonQuery()
                    response = "{ ""response"": """ & sizecolor & " Changed!" & """ }"
                End Using
				Using Command As New SqlCommand("update export set sizecolor=@newsizecolor where company=@company and style=@style and sizecolor=@sizecolor and import_date=@import_date;", connection)
                    Command.Parameters.AddWithValue("@company", company)
                    Command.Parameters.AddWithValue("@style", style)
                    Command.Parameters.AddWithValue("@sizecolor", sizecolor)
					Command.Parameters.AddWithValue("@newsizecolor", newsizecolor)
