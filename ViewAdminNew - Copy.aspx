<%@ Page Language="VB" AutoEventWireup="false" CodeFile="ViewAdminNew - Copy.aspx.vb" Inherits="ViewAdminNew"  MasterPageFile="~/MasterPage.master" MaintainScrollPositionOnPostback="true" %>
...
<asp:Content runat="server" ID="content1" ContentPlaceHolderID="head">
    ...
    originalY = element.offset().top;
            ...
            function get_styles() {
                var result = [];
                handsontable2.loadData(result);
                urlToHandler = 'getView.ashx?company=' + $("#cselect").val() + '&show=' + $("#shselect").val() + "&dn=" + $("#dbselect").val();
                jsonData = '';
                $.ajax({
                    url: urlToHandler,
                    data: jsonData,
            ...
            handsontable2.addHook('afterSelectionEnd', function (row, column) {
                if (column == 3) {
                    ...
                    $.magnificPopup.open({
                        items: {
                            src: '<div class="white-popup"><a style="font-size:2em;" href="javascript:$.magnificPopup.close();">CLOSE</a><br /><iframe style="width:100%;height:100%;" src="' + expimp + ".aspx?company=" + $("#cselect").val() + "&style=" + selectedstyle + "&date=" + date.split(" ")[0] + "&destroyed=" + destroyed + "&fullfile=" + date + '">;</div>',
                            type: 'inline'
                        }
                    });
                }               
            ...
            handsontable2.addHook('afterChange', function (changes, source) {
                ...
                var oldCellValue = changes.toString().split(',')[2];
                var newCellValue = changes.toString().split(',')[3];
                var cell = handsontable2.getCell(row, col);
                var date, stat, comment, cd='';
                if (col == 0) {          
                    cd = oldCellValue;
                    date = newCellValue;
...
<asp:Content runat="server" ID="content3" ContentPlaceHolderID="ContentPlaceHolder2">
    Company: <select id="cselect"></select><span style="margin-left:10px;">Show: <select id="shselect"></select></span> <span style="float:right;">Search: <input  id="search" style="width:100px;"  type="text" /></span> 
